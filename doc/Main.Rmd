...
## Features Extraction
```{python  feature Extraction, eval=FALSE}
def dist(file):
...
```{python Set the model parameters, eval=FALSE}
...
## Cross-Validation
```{python Cross-Validation, eval=FALSE}
train_scores = []
test_scores = []
times = []
X = X_train
...
```{python Predicted Train and Test Score by CVs, eval=FALSE}
