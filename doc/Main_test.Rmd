...
if(testing & run.feature){
  tm_feature_baseline <- system.time(all_test_data_baseline <- feature_baseline(test_fiducial_pt_list, seq(1, length(test_fiducial_pt_list), 1)))
  
  tm_feature_lasso <- system.time(all_test_data_lasso <- feature_regression(test_fiducial_pt_list, seq(1, length(test_fiducial_pt_list), 1)))
...
```{r, eval=FALSE}
labels_prediction <- read.csv(file = paste(test_dir, "labels_prediction.csv", sep=""))
```
