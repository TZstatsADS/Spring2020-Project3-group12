...
testing <- FALSE # run model on a new, never seen before, dataset
```
...
* MODELS EVALUATION
```{r}
tm_test_lasso <- system.time(lasso_accuracy <- regression.cv(models=cv_lasso_models, data=all_train_data, K=K))
tm_test_ridge <- system.time(ridge_accuracy <- regression.cv(models=cv_ridge_models, data=all_train_data, K=K))
cat("The mean training accuracy of the LASSO model is", lasso_accuracy[1]*100, "%.\n")
cat("The mean predicted testing accuracy of the LASSO model is", lasso_accuracy[2]*100, "%.\n")
cat("The mean training accuracy of the Ridge model is", ridge_accuracy[1]*100, "%.\n")
cat("The mean predicted testing accuracy of the Ridge model is", ridge_accuracy[2]*100, "%.\n")
```

Writing to .csv file containing image index and predicted emotion
```{r}
if (testing) {
 lasso_pred <- regression.test(cv_lasso_models, all_test_data)
  ridge_pred <- regression.test(cv_ridge_models, all_test_data)
